...
while True:
  ...
  if command == "print" or command == "print ":
    print(f("{bcolors.FAIL}pysh aborted as it found a bug in your command!\nbug: cannot use only `print', must set input for operation{bcolors.RESET}"))
  elif command == "exit":
    exitconf = input(f("{bcolors.WARNING}Are you sure you want to exit PyShell? (y/n){bcolors.RESET}\nanswer: "))
    if exitconf == "y":
      exit(0)
